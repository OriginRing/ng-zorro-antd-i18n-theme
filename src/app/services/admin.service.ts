...
export class AdminService {
  ...
  verifyToken(token: string | null): Observable<boolean> {
    return this.httpClient.post<{ data: boolean }>(`${url}/api/v1/admin/admin/token`, { token }).pipe(
  ...
  adminToken(userName: string, password: string): Observable<string | null> {
    return this.httpClient
      .post<{ token: string }>(`${url}/api/v1/admin/admin/login`, { userName: userName, passWord: password })
      .pipe(
  ...
  uploadFile(file: FormData): Observable<string> {
    return this.httpClient.post<Upload>(`${url}/api/v1/common/file_upload`, file).pipe(
      map(item => item.info || ''),
      catchError(() => of('123'))
    );
  }
