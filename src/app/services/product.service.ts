...
export class ProductService {
  ...
  ProductGetData(token: string | null, per: number, page: number): Observable<Product>{
    ...
    return this.httpClient.get<Product>(`${url}/api/v1/admin/products?per=${per}&page=${page}`)
      .pipe(
        map(item => item || {}),
        catchError(() => of({}))
      );
  ...
  ProductDelete(id: string | undefined): Observable<boolean>{
    return this.httpClient.delete<ProductCategory>(`${url}/api/v1/admin/products/${id}`)
      .pipe(
        map(() => true || false),
        catchError(() => of(false))
      );
